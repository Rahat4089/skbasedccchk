...
echo $cur;
  ...
  curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
  curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
  curl_setopt($ch, CURLOPT_POSTFIELDS, 'type=card&card[number]=' . $cc . '&card[exp_month]=' . $mm . '&card[exp_year]=' . $yyyy . '&&billing_details[name]=' . $fname . '&billing_details[email]=' . $email . '');
...
function decline_reason($re1, $re2)
{
  ...
  if (strpos($r2, '"seller_message": "Payment complete."')) {
    $status = '#CHARGE';
    $cc_code = $cur . $amount . ' CCN Charged! ';
